...
This script has been adapted for our evaluation but based off of a similar script from UKPLab.
"""
...
from ..util import cos_sim, dot_score, euclidian_dist
import os
import numpy as np
...
class InformationRetrievalEvaluator(SentenceEvaluator):
    ...
    def __init__(self,
                 ...
                 name: str = '',
                 write_csv: bool = True,
                 score_functions: List[Callable[[Tensor, Tensor], Tensor] ] = {'cos_sim': cos_sim, 'dot_score': dot_score,"neg_euclidian_dist":euclidian_dist},       #Score function, higher=more similar
    ...
    def compute_metrices(self, model, corpus_model = None, corpus_embeddings: Tensor = None) -> Dict[str, float]:
        ...
        for corpus_start_idx in trange(0, len(self.corpus), self.corpus_chunk_size, desc='Corpus Chunks', disable=not self.show_progress_bar):
            ...
            for name, score_function in self.score_functions.items():
                ...
                if self.normalize_embeddings: #Normalise the querys and passages with the sub_corpus parameters
                    #print(f"data type sub_corpur {type(sub_corpus_embeddings)} queries {type(query_embeddings)}")

                    #sub_corpus_embeddings=F.normalize(sub_corpus_embeddings,p=2,dim=1)
                    #query_embeddings=F.normalize(query_embeddings,p=2,dim=1)
                    
                    # Calculate mean and standard deviation for sub_corpus_embeddings
                    mean_sub_corpus = sub_corpus_embeddings.mean()
                    std_sub_corpus = sub_corpus_embeddings.std()

                    # Calculate mean and standard deviation for query_embeddings
                    mean_query = query_embeddings.mean()
                    std_query = query_embeddings.std()

                    # Calculate the common normalization parameters
                    common_mean = torch.mean(torch.stack([mean_sub_corpus, mean_query]))
                    common_std = torch.mean(torch.stack([std_sub_corpus, std_query]))

                    # Normalize both sub_corpus_embeddings and query_embeddings using the common normalization parameters
                    sub_corpus_embeddings = (sub_corpus_embeddings - common_mean) / common_std
                    query_embeddings= (query_embeddings - common_mean) / common_std

                pair_scores = score_function(query_embeddings, sub_corpus_embeddings)

                #Get top-k values
                if name == "euclidian_dist":
                    largest=True # changed back because eucidian dist is now negative
                else:
                    largest=True
                pair_scores_top_k_values, pair_scores_top_k_idx = torch.topk(pair_scores, min(max_k, len(pair_scores[0])), dim=1, largest=largest, sorted=False)
