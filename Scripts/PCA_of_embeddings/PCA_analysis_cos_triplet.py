...
#logging information
logging.basicConfig(filename='PCA_cos_triplet.log', filemode='w', format='%(asctime)s - %(message)s', datefmt='%d-%b-%y %H:%M:%S', level=logging.INFO)
...
model_path = "/zhome/d2/a/175738/ML_BERT/DL-SBert/Scripts/triplet_cosine"
...
logging.info(f"Looking at an encoding")
logging.info(model.encode(corp_quer[:6]))
logging.info(f"These are the encodings for texts {corp_quer[:6]}")
...
#plt.grid(True)
plt.show()
plt.savefig('pca_queries_and_corpus_cos_triplet.png')

joblib.dump(pca, 'pca_model_cos_triplet.pkl') 